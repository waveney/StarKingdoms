<?php
  ...
  echo "Properties 1 = Prime, 2= Do projects, 4=Mining Income, 8=Don't have levels, 10=Valid on Space stations, 20=Industrial, " .
       "40=Offices, 80=No Militia, 100=Not built by Industrial, XX000 = Level XX Project (if not 1)<p>\n";

  echo "<form method=post action=DTList.php>";
  Register_AutoUpdate('DistrictTypes', 0);
  ...
  echo "<td><input type=number name=Props:0 >";
  echo "<td>" . fm_select($TechNames,$D,'BasedOn',1,'',"BasedOn:0");
  echo "<td><input type=text name=Gate:0 >";
  echo "<td><input type=number name=MaxNum:0 >";
  echo "<td><input type=text name=Notes:0 >";
  echo "</table></div>\n";

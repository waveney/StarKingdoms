...
//  prof_flag("b4head");
 ...
 //var_dump($T);
  ...
  foreach ($ThingTypes as $Ti=>$TT) {
    if ($TT['Prop2'] & THING_HASNT_DESIGNS) continue;
...
//  prof_flag('Heading');
  echo "<h1>Create or Update a Design</h1>";
  echo "This is to design or modify the design of a ship/$ARMY etc to later build if you want to.";
  ...
  if ($Exist == 0) {
    echo "<form method=post action=PlanDesign.php?ACTION=COPY>";
...
//        var_dump($TT);
        ...
        if ($tprops2 & THING_HASNT_DESIGNS) continue;
...
//var_dump($BPs);
  ...
  echo "<form method=post action=PlanDesign.php >";
  ...
  if ($T['Type']) {
    ...
    echo "<tr>" . fm_text('Design Class Name',$T,'Name') . "$SPad<td" . (empty($T['Name'])? " class=Err" : "") . ">This is needed";
    if (empty($T['Name'])) {
      $Valid = 0;
      $InValid .= "No Name, ";
    ...
    if ($tprops & THING_HAS_BLUEPRINTS ) {
      echo "<tr><td>Blue Print Class:<td>" . $T['Class']  . "$SPad<td colspan=2>" .
        "The final class of a thing made to this design will be [DESIGN NAME] a [BLUEPRINT CLASS].  This is optional";
    } else {
      echo "<tr>" . fm_text('Class',$T,'Class') . "$SPad<td colspan=2>" .
        "The final class of a thing made to this design will be [DESIGN NAME] a [BLUEPRINT CLASS].  This is optional";
    }
...
//    foreach ($MTNs as $Mti=> $MT) {
 ...
 //           var_dump($IsBlue,$min,$max,$FlexM);
      ...
      echo "<tr><td>Evasion:<td>" . $T['Evasion'] . "$SPad<td colspan=3>At current Tech Levels";
...
//            echo "<h2><a href=ThingEdit.php?ACTION=DesCreate&id=$Tid>Create one directly</a></h2>";
      ...
      echo "You can customise by adding crew names, change the image and add notes by selecting the thing while it is being made or used later.<p>\n";
    }
  } else {
    echo "<input type=submit name=Validate value=Validate>\n";
    echo "<input formnovalidate type=submit name=ACTION value=Delete>\n";

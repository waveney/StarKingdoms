...
//  var_dump($_REQUEST);
  ...
  $MaxTechLevel = Feature('MaxTechLevel',5);
...
//    if ($NoC != 1) $Dists[] = ['HostType'=>-1, 'HostId' => $PH['id'], 'Type'=> -1, 'Number'=>0, 'id'=>-$PH['id']];
  ...
  if ($GM) echo "GM: To build over the limits add &OVERRIDE to the URL<p>";
...
//var_dump($DTz,$CProjs);
 ...
 //         if ($DTz['MaxNum']<$D['Number']+1) continue;
    ...
    echo "<h2>Research Supplemental Technology</h2><table class=ProjTab border><th>Project<th>Cost<th>Progress<br>Needed<th>Description";
      ...
      foreach ($SpecTechs as $T) {
        if ($FactTechs[$T['id']]['Level']??0) {
          ...
          $Lvl = $FactTechs[$T['id']]['Level']+1;
          $pc = Proj_Costs($Lvl);
          Show_Research($T['Name'] . " Level $Lvl","ProjDisp.php?ACTION=NEW&id=$Fid&p=" . $PTi['Research Core Technology'] .
            "&t=$Turn&Hi=$Hi&Di=$Di&DT=$DT&Sel=" . $T['id'] .
            "&Name=" . base64_encode("Research " . $T['Name'] . $Place) . "&L=$Lvl&C=" . $pc[1] . "&PN=" . $pc[0],
            $pc[1],$pc[0],$T['Description']);
        }
...
//    echo "Not in this game<p>";
      ...
      if (Has_Trait($Fid,"It's worth taking a long time to say it")) {
      ...
      if (Has_Tech($Fid,'Flux Crystal Synthesis')) { /// Should drive from projects but this is a quick hack
        echo "<h2>Flux Crystal Synthesis</h2>";
        $Lvl = 1;
        $pc = Proj_Costs($Lvl);
      ...
      if (Has_Tech($Fid,'Cret-Chath Synthesis')) { /// Should drive from projects but this is a quick hack
        echo "<h2>Cret-Chath Synthesis</h2>";
        $Lvl = 1;
        $pc = Proj_Costs($Lvl);
        echo "<form method=post><button class=projtype type=submit formaction='ProjDisp.php?ACTION=NEW&id=$Fid&p=" .

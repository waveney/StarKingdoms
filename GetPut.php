...
function Get_Faction_Colours() {
  global $db,$GAMEID;
  $res = $db->query("SELECT id,MapColour,MapText FROM Factions WHERE GameId=$GAMEID ORDER BY id ");
  $F = $T = [];
  $F[0] = "white";
  $T[0] = 'black';
  if ($res) {
    while ($ans = $res->fetch_assoc()) {
      $F[$ans['id']] = $ans['MapColour'];
      $T[$ans['id']] = $ans['MapText'];
    }
  }
  return [$F,$T];
}
...
function Get_ThingsSys($Sid,$type=0,$Fid=0) {
  global $db,$GAMEID;
  $Ts = [];
  $res = $db->query("SELECT * FROM Things WHERE GameId=$GAMEID AND SystemId=$Sid AND ( LinkId>=0 OR LinkId<" . LINK_LOAD_AND_UNLOAD .
    ") " . ($type?" AND Type=$type":"") . ($Fid?" AND Whose=$Fid":""));
  if ($res) while ($ans = $res->fetch_assoc()) $Ts[] = $ans;
  return $Ts;
}
...
function Get_Techs($Fact=0,$AllG=0) {
  ...
  $Ms = [];
  $res = $db->query("SELECT * FROM Technologies " . ($AllG?'':"WHERE (NotBy=0 OR (NotBy&$NOTBY)!=0) ") . " ORDER BY Cat,Name");
  if ($res) while ($ans = $res->fetch_assoc()) $Ms[$ans['id']] = $ans;
  return $Ms;
}
